...
( set -x; docker run -t  -e "MAKEOPTS=-j$(( $( nproc ) +1))" -e "EMERGE_DEFAULT_OPTS='-kg'" -v /usr/portage:/usr/portage:ro -v /usr/portage/distfiles:/usr/portage/distfiles --name "$container" "$image" bash -exc $'
...
echo 'COPY configs/portage/ /etc/portage' >> Dockerfile
